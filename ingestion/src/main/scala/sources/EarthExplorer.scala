...
class EarthExplorerWork(override val source: EarthExplorerSource,
  ...
  override val url =
    s"""${source.baseUrl}search?jsonRequest={"apiKey":"7406a0517daa437f823ce80e7b79e5c7","datasetName":"${source.productType}","temporalFilter":{"startDate":"${ingestionDates._1.toString(dateFormat)}","endDate":"${ingestionDates._2.toString(dateFormat)}"},"includeUnknownCloudCover":true,"maxResults":"${source.pageSize}","startingNumber":"$pageStart","sortOrder":"DESC"}"""
...
//    getNextPagesWork(doc).foreach(w => workToBeDone ::= w)
  ...
  private def generateEEMetadataWork(entityId: String) = {
    val mdUrl = s"""${source.baseUrl}metadata?jsonRequest={"apiKey":"7406a0517daa437f823ce80e7b79e5c7","datasetName":"${source.productType}","entityIds":"$entityId"}"""
